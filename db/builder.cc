...
Status BuildTable(
    ...
    bool skip_sync, std::vector<std::string>* files_to_sync) {
  assert((tboptions.column_family_id ==
          TablePropertiesCollectorFactory::Context::kUnknownColumnFamily) ==
         tboptions.column_family_name.empty());
  ...
  // jw-perf: BuildTable phase monitoring
  static std::ofstream build_log;
  static std::mutex build_mutex;
  static bool build_init = false;
  if (!build_init) {
    std::lock_guard<std::mutex> lock(build_mutex);
    if (!build_init) {
      build_log.open("buildtable_perf.csv");
      build_log << "timestamp_ms,phase,duration_us,bytes\n";
      build_init = true;
    }
  }
  ...
  uint64_t phase_start = db_options.clock->NowMicros();
  auto record_build_phase = [&](const std::string& phase, uint64_t bytes = 0) {
    uint64_t phase_end = db_options.clock->NowMicros();
    std::lock_guard<std::mutex> lock(build_mutex);
    build_log << get_timestamp_ms() << ","
              << phase << ","
  ...
  if (iter->Valid() || !range_del_agg->IsEmpty()) {
    ...
    record_build_phase("file_creation_complete");
    ...
    record_build_phase("iterator_setup_complete");
    ...
    record_build_phase("data_iteration_complete");
    ...
    if (!s.ok() || empty) {
      ...
      record_build_phase("range_tombstones_complete");

      s = builder->Finish();
      record_build_phase("builder_finish_complete");
    }
    if (io_status->ok()) {
    ...
    record_build_phase("metadata_update_complete");
    ...
    *io_status =
    ...
    //jw: When skip_sync=true, defer sync to caller for batch sync optimization
    //    This reduces N fsyncs to 1 fsync when splitting memtable into N files
    if (s.ok() && io_status->ok() && !empty) {
      if (!skip_sync) {
        StopWatch sw(ioptions.clock, ioptions.stats, TABLE_SYNC_MICROS);
        *io_status = file_writer->Sync(opts, ioptions.use_fsync);
        record_build_phase("file_sync_complete");
      } else {
        //jw: Record file path for later batch sync
        if (files_to_sync != nullptr) {
          files_to_sync->push_back(fname);
        }
        record_build_phase("file_sync_skipped");
      }
    }
    TEST_SYNC_POINT("BuildTable:BeforeCloseTableFile");
    if (s.ok() && io_status->ok() && !empty) {
      //jw: Pass skip_sync to Close() to also skip Fsync in direct I/O mode
      *io_status = file_writer->Close(opts, skip_sync);
      record_build_phase("file_close_complete");
    }
    if (s.ok() && io_status->ok() && !empty) {
