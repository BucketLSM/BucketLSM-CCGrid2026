...
#include <atomic>
#include <deque>   //jw: for Bucket::recent_flushes
#include <mutex>   //jw: for BucketManager::mutex_
#include <string>
...
//jw: Forward declaration
class BucketManager;

extern const double kIncSlowdownRatio;

//jw: ============================================================================
//jw: Bucket-based flush data structures (use_new_flush=1)
//jw: ============================================================================

//jw: Single flush event record for window-based statistics
// Represents what happened to a bucket during one global flush
struct FlushEvent {
  uint64_t flush_id;        //jw: Global flush ID (key for tracking)
  uint64_t file_count;      //jw: Number of files this bucket received (can be 0)
  uint64_t entry_count;     //jw: Entries flushed to this bucket (can be 0)
  uint64_t data_bytes;      //jw: Bytes flushed to this bucket (can be 0)
  std::string median_key;   //jw: Median key of this flush (for split point calc)
};

//jw: Bucket represents a key range partition for flush splitting
struct Bucket {
  std::string min_key;  // inclusive (empty = -infinity)
  std::string max_key;  // exclusive (empty = +infinity)

  //jw: Window-based statistics using map (flush_id -> event)
  // All buckets track the same global flush IDs (last WINDOW_SIZE flushes)
  std::map<uint64_t, FlushEvent> flush_events;
  static constexpr size_t WINDOW_SIZE = 10;

  //jw: Incremental aggregates for window (updated on add/remove)
  // These represent the sum over the last WINDOW_SIZE global flushes
  uint64_t window_file_count = 0;
  uint64_t window_entry_count = 0;
  uint64_t window_data_bytes = 0;

  //jw: Add a new flush event (called for every global flush, even if file_count=0)
  void AddFlushEvent(uint64_t flush_id, uint64_t file_count, uint64_t entries,
                     uint64_t bytes, const std::string& median_key);

  //jw: Remove an old flush event (when window slides)
  void RemoveFlushEvent(uint64_t flush_id);

  //jw: Recalculate window stats from scratch (used after split/merge)
  void RecalculateWindowStats();
};

//jw: Result of flushing to a single bucket (used by OnFlushCompleted)
struct BucketFlushResult {
  uint64_t file_count;      //jw: Number of files created for this bucket (can be 0)
  uint64_t entry_count;     //jw: Number of entries flushed to this bucket (can be 0)
  uint64_t data_bytes;      //jw: Total bytes written to this bucket (can be 0)
  std::string median_key;   //jw: Median key of entries flushed to this bucket
  std::string min_key;      //jw: Min key of this bucket at flush time (for remapping)
  std::string max_key;      //jw: Max key of this bucket at flush time (for remapping)
};

// BucketManager manages global bucket partitions for a column family
class BucketManager {
 public:
  //jw: Lightweight snapshot for optimistic concurrency control
  // Flush operations acquire a snapshot once and work with it lock-free
  struct BucketSnapshot {
    uint64_t epoch;  // Structure version at snapshot time
    std::vector<std::pair<std::string, std::string>> ranges;  // (min_key, max_key)

    size_t GetBucketCount() const { return ranges.size(); }
    const std::string& GetMinKey(size_t idx) const { return ranges[idx].first; }
    const std::string& GetMaxKey(size_t idx) const { return ranges[idx].second; }
  };

  //jw: Constructor takes optional ColumnFamilyData for accessing options
  explicit BucketManager(int target_count, ColumnFamilyData* cfd = nullptr);
  ~BucketManager() = default;

  // Check if buckets are initialized
  bool IsInitialized() const { return initialized_; }

  // Initialize buckets from first flush data
  // split_points: C-1 keys that divide key space into C buckets
  void InitializeFromFirstFlush(const std::vector<std::string>& split_points,
                                 uint64_t entries_per_bucket);

  // Find which bucket a key belongs to
  // Returns bucket index (0 to bucket_count-1)
  int FindBucket(const std::string& key) const;

  //jw: Get snapshot of current bucket structure (single lock acquisition)
  // Use this at the start of flush to avoid repeated locking
  BucketSnapshot GetSnapshot() const;

  //jw: Get current epoch (lock-free)
  // Used to check if structure changed during flush
  uint64_t GetCurrentEpoch() const {
    return structure_epoch_.load(std::memory_order_acquire);
  }

  //jw: Find bucket in snapshot (no lock needed - static helper)
  static int FindBucketInSnapshot(const BucketSnapshot& snap,
                                  const std::string& key);

  // Get bucket count
  size_t GetBucketCount() const { return buckets_.size(); }

  // Get bucket by index
  Bucket& GetBucket(size_t idx) { return buckets_[idx]; }
  const Bucket& GetBucket(size_t idx) const { return buckets_[idx]; }
  ...
  // This is the main entry point for window management
  // snapshot_epoch: epoch value when flush started (for detecting concurrent split/merge)
  // results: per-bucket flush results (size may differ from current bucket count)
  void OnFlushCompleted(uint64_t snapshot_epoch,
                        const std::vector<BucketFlushResult>& results);

  // Adjust buckets after compaction (split/merge logic)
  // Returns true if any bucket was modified
  bool AdjustAfterCompaction(int compacted_bucket_idx);

  //jw: Adjust all buckets that overlap with the given key range
  // Called after compaction to check all affected buckets for split/merge
  // Returns true if any bucket was modified
  bool AdjustBucketsInRange(const std::string& min_key, const std::string& max_key);

  //jw: Find all bucket indices that overlap with the given key range
  std::vector<int> FindBucketsInRange(const std::string& min_key,
                                      const std::string& max_key) const;

  //jw: Get all bucket boundary keys (for compaction output splitting)
  // Returns sorted list of boundary keys (excludes -inf and +inf)
  std::vector<std::string> GetAllBoundaries() const;

  //jw: Check if a key is exactly at a bucket boundary
  bool IsBucketBoundary(const std::string& key) const;

  // Get target bucket count
  int GetTargetBucketCount() const { return target_bucket_count_; }

  // Get bucket boundaries for logging/debugging
  std::string GetBucketBoundariesString() const;

 private:
  // Split bucket at given index
  void SplitBucket(int idx);

  // Merge bucket with adjacent bucket
  void MergeWithAdjacentBucket(int idx);

  // Rebalance to maintain target bucket count
  void RebalanceToTargetCount();

  // Compute split point from bucket's recent flush data
  std::string ComputeSplitPoint(const Bucket& b) const;

  // Compute lexicographic midpoint of two keys
  std::string ComputeMidpointKey(const std::string& a, const std::string& b) const;

  //jw: Internal find bucket (assumes mutex_ is already held)
  int FindBucketInternal(const std::string& key) const;

  //jw: Internal version of FindBucketsInRange (assumes mutex_ is already held)
  // Used by OnFlushCompleted slow path for remapping
  std::vector<int> FindBucketsInRangeInternal(const std::string& min_key,
                                               const std::string& max_key) const;

  std::vector<Bucket> buckets_;
  int target_bucket_count_;
  bool initialized_ = false;
  mutable std::mutex mutex_;
  //jw: Epoch counter for optimistic concurrency - incremented on Split/Merge
  std::atomic<uint64_t> structure_epoch_{0};
  //jw: Global flush counter - incremented on each flush completion
  // All buckets track statistics based on this counter (last WINDOW_SIZE flushes)
  uint64_t global_flush_counter_ = 0;
  //jw: Optional pointer to ColumnFamilyData for accessing options
...
class ColumnFamilyData {
 public:
  ...
  //jw: Get the number of L0 groups that need compaction (for parallel L0-L1 compaction)
  // REQUIRES: DB mutex held
  int GetNumGroupsNeedingCompaction() const;
  // REQUIRES: DB mutex held
  ...
  // Record write stall time based on the cause and condition
  void RecordWriteStallTime(uint64_t elapsed_micros);
  ...
  //jw: Pending bucket files API for concurrent flush coordination (used by bucket flush)
  // REQUIRES: db_mutex_ held
  void RegisterPendingSplitFiles(
      uint64_t job_id,
      const std::vector<std::pair<std::string, std::string>>& file_ranges);

  // REQUIRES: db_mutex_ held
  void UnregisterPendingSplitFiles(uint64_t job_id);

  // REQUIRES: db_mutex_ held
  std::vector<std::pair<std::string, std::string>> GetAllPendingSplitFileRanges() const;

  //jw: Pending compaction files API for concurrent intra-L0 compaction coordination
  // Used when intra-L0 compaction (output_level == start_level == 0) is in progress
  // REQUIRES: db_mutex_ held
  void RegisterPendingCompactionFiles(
      uint64_t job_id,
      const std::vector<std::pair<std::string, std::string>>& file_ranges);

  // REQUIRES: db_mutex_ held
  void UnregisterPendingCompactionFiles(uint64_t job_id);

  // REQUIRES: db_mutex_ held
  std::vector<std::pair<std::string, std::string>> GetAllPendingCompactionFileRanges() const;

  //jw: BucketManager API for bucket-based flush (use_new_flush=1)
  // Get the bucket manager (creates one if doesn't exist)
  // target_bucket_count: initial_split_count from options
  BucketManager* GetBucketManager(int target_bucket_count);

  // Check if bucket manager exists and is initialized
  bool HasInitializedBucketManager() const;
 ...
 private:
  ...
  // Track write stall start time and cause for duration measurement
  uint64_t write_stall_start_time_;
  WriteStallCause write_stall_cause_;
  WriteStallCondition write_stall_condition_;
  ...
  //jw: Pending split file structure for concurrent compaction coordination
  struct PendingSplitFile {
    uint64_t job_id;
    uint32_t file_index;  // Index within this flush job (0, 1, 2, ...)
    std::string smallest_key;
    std::string largest_key;
  };

  // REQUIRES: db_mutex_ held
  std::map<uint64_t, std::vector<PendingSplitFile>> pending_split_files_;

  //jw: For intra-L0 compaction output files
  // This prevents incorrect splitting when intra-L0 compactions run concurrently
  // REQUIRES: db_mutex_ held
  std::map<uint64_t, std::vector<PendingSplitFile>> pending_compaction_files_;

  //jw: BucketManager for bucket-based flush (use_new_flush=1)
  // Manages global key-space partitioning for uniform flush distribution
