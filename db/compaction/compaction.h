...
class Compaction {
 public:
  ...
  //jw: L0 group boundary structure for aligning L1 output files
  struct L0GroupBoundary {
    std::string min_key;  // Smallest key of the group
    std::string max_key;  // Largest key of the group

    L0GroupBoundary(std::string min, std::string max)
        : min_key(std::move(min)), max_key(std::move(max)) {}
  };

  //jw: L0 group boundary accessors for parallel L0→L1 compaction
  const std::vector<L0GroupBoundary>& GetL0GroupBoundaries() const {
    return l0_group_boundaries_;
  }
  void SetL0GroupBoundaries(std::vector<L0GroupBoundary>&& boundaries) {
    l0_group_boundaries_ = std::move(boundaries);
  }
 ...
 private:
  ...
  //jw: L0 group boundaries for aligning L1 output files (use_new_flush only)
  //    Contains min_key and max_key of each L0 group (sorted by min_key)
  std::vector<L0GroupBoundary> l0_group_boundaries_;
