...
#include "db/column_family.h"  //jw: for BucketManager
...
void CompactionJob::GenSubcompactionBoundaries() {
  ...
  //jw: Debug subcompaction boundary generation
  ROCKS_LOG_INFO(db_options_.info_log,
      "[JW_SUBCOMPACTION] GenSubcompactionBoundaries completed: "
      "input_level=%d, output_level=%d, "
      "num_planned_subcompactions=%lu, num_actual_subcompactions=%lu, "
      "boundaries_count=%zu",
      c->start_level(),
      c->output_level(),
      num_planned_subcompactions,
...
static void LogL0CoverageSegments(
    const std::vector<const FileMetaData*>& files,
    const InternalKeyComparator& icmp,
    Logger* info_log,
    EventLogger* event_logger,
    int job_id,
    const char* tag,                 // e.g., "CPT-IN", "CPT-AFTER"
    size_t max_segments_to_log = 200) {
  if (files.empty()) return;

  std::vector<SegmentEvt> ev;
  ev.reserve(files.size() * 2);
  for (auto* f : files) {
    ev.push_back({ &f->smallest, f->fd.GetNumber(), +1 });
    ev.push_back({ &f->largest,  f->fd.GetNumber(), -1 });
  }
  std::sort(ev.begin(), ev.end(), [&](const SegmentEvt& a, const SegmentEvt& b){
    int c = icmp.Compare(*a.k, *b.k);
    if (c != 0) return c < 0;
    return a.delta > b.delta; // For equal keys, process start (+1) before end
  });

  std::vector<uint64_t> active; active.reserve(files.size());
  auto add = [&](uint64_t n){ active.push_back(n); };
  auto del = [&](uint64_t n){ auto it=std::find(active.begin(),active.end(),n);
                              if(it!=active.end()) active.erase(it); };

  size_t seg = 0;
  for (size_t i=0; i+1<ev.size(); ++i) {
    (ev[i].delta>0 ? add(ev[i].file_no) : del(ev[i].file_no));
    if (icmp.Compare(*ev[i].k, *ev[i+1].k) != 0) {
      if (seg < max_segments_to_log) {
        std::vector<uint64_t> here = active;
        std::sort(here.begin(), here.end());

        if (!here.empty()) { // Skip logging for empty segments
          ROCKS_LOG_INFO(info_log,
            "[L0Coverage-%s] seg=%zu depth=%zu range=[%s..%s) files=%s",
            tag, seg, here.size(),
            ev[i].k->user_key().ToString(true).c_str(),
            ev[i+1].k->user_key().ToString(true).c_str(),
            [&]{ std::string s; s.push_back('{');
                 for (size_t t=0; t<here.size(); ++t) {
                   if (t) s.append(", ");
                   s.append(std::to_string(here[t]));
                 }
                 s.push_back('}'); return s; }().c_str());

          if (event_logger) {
            event_logger->Log()
              << "job" << job_id
              << "event" << "l0_coverage_segment"
              << "phase" << tag
              << "seg_index" << static_cast<int>(seg)
              << "depth" << static_cast<int>(here.size())
              << "begin" << ev[i].k->user_key().ToString(false)
              << "end"   << ev[i+1].k->user_key().ToString(false);
          }
        }
      }
      ++seg;
  ...
  ROCKS_LOG_INFO(info_log,
    "[L0Coverage-%s] segments_total=%zu (logged up to %zu), files=%zu",
    tag, seg, std::min(seg, max_segments_to_log), files.size());
}
...
Status CompactionJob::Run() {
  ...
  //jw: Debug subcompaction thread usage
  ROCKS_LOG_INFO(db_options_.info_log,
      "[JW_SUBCOMPACTION] Compaction starting: "
      "input_level=%d, output_level=%d, "
      "max_subcompactions(configured)=%d, actual_subcompactions(threads)=%zu",
  ...
  compact_->compaction->GetOrInitInputTableProperties();
   ...
   //jw: Log L0 compaction input SSTs (CPT-IN) coverage segments
  if (compact_->compaction->start_level() == 0) {
    const auto* l0_vec = compact_->compaction->inputs(/*which=*/0);
    std::vector<const FileMetaData*> l0_inputs;
    l0_inputs.reserve(l0_vec ? l0_vec->size() : 0);
    if (l0_vec) {
      for (auto* f : *l0_vec) l0_inputs.push_back(f);
    }
  
    LogL0CoverageSegments(
        l0_inputs,
        ...
        job_context_->job_id,
        /*tag=*/"CPT-IN",
        /*max_segments_to_log=*/10000000000);
  }
...
Status CompactionJob::Install(bool* compaction_released) {
  ...
  stream << "job" << job_id_ << "event" << "compaction_finished"
         ...
         //jw: Add start_level and input_data_size for L0-L1 compaction analysis
         << "start_level" << compact_->compaction->start_level()
         << "output_level" << compact_->compaction->output_level()
         << "input_data_size" << (bytes_read_non_output_and_blob + stats.bytes_read_output_level)
         << "num_output_files" << stats.num_output_files
         << "total_output_size" << stats.bytes_written;
...
Status CompactionJob::InstallCompactionResults(bool* compaction_released) {
  ...
  //jw: Bucket adjustment after L0 compaction (use_new_flush=1)
  // If this compaction started from L0 and bucket manager is initialized,
  // adjust ONLY buckets that actually had files compacted (not all in range)
  if (compaction->start_level() == 0 &&
      compaction->column_family_data()->HasInitializedBucketManager()) {
    auto* cfd = compaction->column_family_data();
    auto* bucket_mgr = cfd->GetBucketManager(
        compaction->immutable_options().initial_split_count);

    if (bucket_mgr && bucket_mgr->IsInitialized()) {
      const auto* input_files = compaction->inputs(0);  // L0 input files
      if (input_files && !input_files->empty()) {
        //jw: Collect unique bucket indices that actually had files compacted
        // Use a set to avoid duplicate adjustments for the same bucket
        std::set<int> compacted_buckets;
        for (const auto* file : *input_files) {
          // Use the file's smallest key as representative to find bucket
          std::string rep_key = file->smallest.user_key().ToString();
          int bucket_idx = bucket_mgr->FindBucket(rep_key);
          if (bucket_idx >= 0) {
            compacted_buckets.insert(bucket_idx);
          }
        }

        //jw: Adjust only the buckets that actually had files compacted
        bool any_adjusted = false;
        for (int bucket_idx : compacted_buckets) {
          bool adjusted = bucket_mgr->AdjustAfterCompaction(bucket_idx);
          if (adjusted) {
            any_adjusted = true;
            ROCKS_LOG_BUFFER(log_buffer_,
                "[%s] [JOB %d] Bucket %d adjusted after compaction",
                cfd->GetName().c_str(), job_id_, bucket_idx);
          }
        }
        if (any_adjusted) {
          ROCKS_LOG_BUFFER(log_buffer_,
              "[%s] [JOB %d] New bucket boundaries: %s",
