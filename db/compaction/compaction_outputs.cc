...
bool CompactionOutputs::ShouldStopBefore(const CompactionIterator& c_iter) {
  ...
  //jw: For L0→L1 compaction with use_new_flush, align output files based on bucket boundaries
  //    use_new_flush=1 (bucket mode): Use bucket boundaries from BucketManager
  if (compaction_->start_level() == 0 && compaction_->output_level() == 1 &&
      compaction_->immutable_options().use_new_flush != 0) {

    Slice user_key = ExtractUserKey(internal_key);
    const Comparator* ucmp = icmp->user_comparator();

    //jw: Calculate size thresholds: 50-200% of target size
    const uint64_t target_size = compaction_->target_output_file_size();
    const uint64_t min_split_size = 0;      // 50% of target
    const uint64_t max_split_size = (target_size * 110)/100;      // 200% of target

    //jw: Only split at boundary if current file size is within acceptable range
    //    This prevents creating too small files (< 50%) or too large files (> 200%)
    if (current_output_file_size_ >= min_split_size &&
        current_output_file_size_ <= max_split_size) {

      //jw: use_new_flush=1 (bucket mode): Split at bucket boundaries
      // Use pre-acquired snapshot (set in constructor) for lock-free lookups
      if (compaction_->immutable_options().use_new_flush == 1 && bucket_snapshot_) {
        //jw: Find which bucket this key belongs to (lock-free with snapshot)
        int bucket_idx = BucketManager::FindBucketInSnapshot(
            *bucket_snapshot_, std::string(user_key.data(), user_key.size()));

        //jw: Check if we've moved to a different bucket
        if (current_bucket_index_ == -1) {
          //jw: First key - initialize tracking, don't split
          current_bucket_index_ = bucket_idx;
        } else if (bucket_idx != current_bucket_index_) {
          //jw: Bucket changed - split here and update tracking
          current_bucket_index_ = bucket_idx;
          return true;
        }
        //jw: Same bucket - continue accumulating
      }
    }
  }
...
CompactionOutputs::CompactionOutputs(const Compaction* compaction,
  ...
  //jw: Acquire bucket snapshot once at compaction start for use_new_flush=1 (bucket mode)
  // This avoids repeated locking during compaction
  if (compaction->start_level() == 0 && compaction->output_level() == 1 &&
      compaction->immutable_options().use_new_flush == 1) {
    auto* cfd = compaction->column_family_data();
    if (cfd && cfd->HasInitializedBucketManager()) {
      ...
      if (bucket_mgr && bucket_mgr->IsInitialized()) {
        bucket_snapshot_ = std::make_shared<BucketManager::BucketSnapshot>(
            bucket_mgr->GetSnapshot());
      }
    }
  }
