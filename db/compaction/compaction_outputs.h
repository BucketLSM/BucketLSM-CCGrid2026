...
class CompactionOutputs {
 ...
 private:
  ...
  //jw: Bucket boundary tracking for use_new_flush=1 (bucket mode)
  // Snapshot acquired once at compaction start for lock-free bucket lookups
  std::shared_ptr<BucketManager::BucketSnapshot> bucket_snapshot_;
  // Track which bucket the current key belongs to, split when it changes
  int current_bucket_index_ = -1;  // -1 = not initialized
