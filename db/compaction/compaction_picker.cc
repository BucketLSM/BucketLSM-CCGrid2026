...
bool CompactionPicker::SetupOtherInputs(
  ...
  //
  //jw: For L0 with use_new_flush (1=split, 2=bucket), skip input expansion entirely.
  //    Reason: Union-Find in PickFileToCompact() already selected the complete
  //    overlapping group. Expanding here based on L0+L1 combined range could
  //    pull in L0 files from OTHER groups, breaking parallel L0 compaction design.
  //    SetupOtherL0FilesIfNeeded() is already skipped for use_new_flush,
  //    so we must also skip this second expansion attempt.
  if (input_level == 0 && ioptions_.use_new_flush != 0) {
    return true;  // Skip L0 expansion - Union-Find group is complete
  }
...
bool CompactionPicker::GetOverlappingL0Files(
  ...
  // NOTE: With use_new_flush=1, this may fail due to concurrent L0 compactions
