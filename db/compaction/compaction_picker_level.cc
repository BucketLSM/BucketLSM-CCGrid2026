...
#include "logging/logging.h"  //jw: For ROCKS_LOG_INFO
...
class LevelCompactionBuilder {
 public:
  ...
  //jw: Detect L0 group boundaries for aligning L1 output files
  std::vector<Compaction::L0GroupBoundary> DetectL0GroupBoundaries();
...
void LevelCompactionBuilder::PickFileToCompact(
  ...
  for (auto& level_file : level_files) {
    ...
    //jw: Allow parallel L0 compactions when use_new_flush=1 or 2
    //    Otherwise, maintain original behavior (serialize L0 compactions)
    if (start_level_ == 0 && ioptions_.use_new_flush == 0 &&
...
void LevelCompactionBuilder::SetupInitialFiles() {
  ...
  // Log all level scores for debugging
  std::string all_scores;
  for (int level = 0; level < compaction_picker_->NumberLevels() - 1; level++) {
    if (level > 0) all_scores += ", ";
    all_scores += "L" + std::to_string(vstorage_->CompactionScoreLevel(level)) +
                  ":" + std::to_string(vstorage_->CompactionScore(level));
  }
  ROCKS_LOG_INFO(ioptions_.logger,
                 "[%s] PickCompaction: Level scores (sorted by priority): [%s]",
                 cf_name_.c_str(), all_scores.c_str());
  ...
  for (int i = 0; i < compaction_picker_->NumberLevels() - 1; i++) {
    ...
    if (start_level_score_ >= 1) {
      if (skipped_l0_to_base && start_level_ == vstorage_->base_level()) {
        ...
        // may starve.
        ROCKS_LOG_INFO(ioptions_.logger,
                       "[%s] Skipping L%d->L%d compaction (score=%.2f) "
                       "to avoid starving L0->base compaction",
                       cf_name_.c_str(), start_level_,
                       (start_level_ == 0) ? vstorage_->base_level() : start_level_ + 1,
                       start_level_score_);
        continue;
      }
      ...
      ROCKS_LOG_INFO(ioptions_.logger,
                     "[%s] Attempting L%d->L%d compaction (score=%.2f, priority=%d)",
                     cf_name_.c_str(), start_level_, output_level_,
                     start_level_score_, i);
      ...
      if (picked_file_to_compact) {
        ...
        ROCKS_LOG_INFO(ioptions_.logger,
                       "[%s] *** PICKED L%d->L%d compaction (score=%.2f) ***",
                       cf_name_.c_str(), start_level_, output_level_,
                       start_level_score_);
        if (start_level_ == 0) {
        ...
        ROCKS_LOG_INFO(ioptions_.logger,
                       "[%s] Could not pick L%d->L%d compaction (score=%.2f) - "
                       "files may be in-use or conflicting",
                       cf_name_.c_str(), start_level_, output_level_,
                       start_level_score_);
        ...
        if (start_level_ == 0) {
          ...
          if (PickIntraL0Compaction()) {
            ROCKS_LOG_INFO(ioptions_.logger,
                           "[%s] *** PICKED L0->L0 intra-level compaction "
                           "(score=%.2f) as fallback ***",
                           cf_name_.c_str(), start_level_score_);
            output_level_ = 0;
            compaction_reason_ = CompactionReason::kLevelL0FilesNum;
            break;
          }
...
bool LevelCompactionBuilder::SetupOtherL0FilesIfNeeded() {
  if (start_level_ == 0 && output_level_ != 0 && !is_l0_trivial_move_) {
    //jw: For use_new_flush (1=split, 2=bucket), PickFileToCompact() already selected
    //    the complete overlapping group using Union-Find with User Key comparison.
    //    Calling GetOverlappingL0Files() would:
    //    1. Clear the already-selected files
    //    2. Re-select using GetOverlappingInputs (which doesn't check being_compacted)
    //    This can cause the same file to be included in multiple compactions.
    //    So we skip this step for use_new_flush.
    if (ioptions_.use_new_flush != 0) {
      return true;  // Already have complete group from PickFileToCompact()
    }
...
bool LevelCompactionBuilder::SetupOtherInputsIfNeeded() {
  ...
  if (output_level_ != 0) {
    ...
    if (compaction_picker_->FilesRangeOverlapWithCompaction(
      ...
      //jw: Log the conflict for analysis
      InternalKey smallest, largest;
      compaction_picker_->GetRange(compaction_inputs_, &smallest, &largest,
                                    Compaction::kInvalidLevel);
      ROCKS_LOG_INFO(ioptions_.logger,
                     "[%s] Compaction conflict: L%d->L%d key range [%s, %s] "
                     "overlaps with running compaction, skipping",
                     cf_name_.c_str(), start_level_, output_level_,
...
Compaction* LevelCompactionBuilder::GetCompaction() {
  ...
  //jw: For L0→L1 compaction with use_new_flush (1=split, 2=bucket), detect L0 groups and set boundaries
  if (start_level_ == 0 && output_level_ == 1 && ioptions_.use_new_flush != 0) {
    std::vector<Compaction::L0GroupBoundary> boundaries = DetectL0GroupBoundaries();
    c->SetL0GroupBoundaries(std::move(boundaries));
  }
  ...
  if (start_level_ == 0 && mutable_cf_options_.compression_per_level.empty() &&
    ...
    for (auto it = level_files.rbegin(); it != level_files.rend(); ++it) {
      ...
      vstorage_->GetOverlappingInputs(output_level_, &my_smallest, &my_largest,
                                      &output_level_inputs.files);
      if (output_level_inputs.empty()) {
        // NOTE: With use_new_flush=1, files may be concurrently compacted
...
bool LevelCompactionBuilder::PickFileToCompact() {
  ...
  //jw: When use_new_flush=1 or 2, allow parallel L0→L1 compactions
  //    Skip this early return check to reach the group selection logic below
  if (start_level_ == 0 && ioptions_.use_new_flush == 0 &&
      !compaction_picker_->level0_compactions_in_progress()->empty()) {
  ...
  //jw: For L0 with use_new_flush (1=split, 2=bucket), pick an available overlapping group
  //    use_new_flush=1: Groups are formed by overlapping key ranges (split-based flush)
  //    use_new_flush=1: Groups are effectively bucket-aligned because:
  //      - Bucket-aligned flush creates L0 files within bucket boundaries
  //      - Files in the same bucket have overlapping key ranges → grouped together
  //      - Files in different buckets have non-overlapping ranges → separate groups
  //    This approach correctly handles bucket boundary changes (split/merge):
  //      - Split: Old files spanning new boundaries will be grouped and realigned by output
  //      - Merge: Old files remain separate groups but output alignment converges over time
  if (start_level_ == 0 && ioptions_.use_new_flush != 0) {
    // Detect all L0 groups using Union-Find
    // IMPORTANT: Exclude being_compacted files and use User Key comparison
    const std::vector<FileMetaData*>& l0_files = vstorage_->LevelFiles(0);
    const size_t num_files = l0_files.size();
    ...
    if (num_files > 0) {
      ...
      if (!active_indices.empty()) {
        // Union-Find to detect groups (only among active files)
        ...
        const Comparator* ucmp = vstorage_->InternalComparator()->user_comparator();
        ...
        // Group files by root (only active files)
        std::map<int, std::vector<FileMetaData*>> groups;
        for (size_t idx : active_indices) {
          groups[find(static_cast<int>(idx))].push_back(l0_files[idx]);
        }

        //jw: If l0_weight_compaction is enabled, use weighted score
        //    Score = α * (N / N_max) + (1 - α) * (1 - R / R_max)
        //    where α = l0_file_count_weight
        if (ioptions_.l0_weight_compaction) {
          struct GroupScore {
            std::vector<FileMetaData*> files;
            size_t file_count = 0;
            uint64_t group_size = 0;
            uint64_t l1_overlap_size = 0;
            double overlap_ratio = 0.0;
            double score = 0.0;
          };
          std::vector<GroupScore> scored_groups;

          const InternalKeyComparator* icmp = vstorage_->InternalComparator();

          // Calculate metrics for each group
          for (auto& kv : groups) {
            GroupScore gs;
            gs.files = std::move(kv.second);
            gs.file_count = gs.files.size();
            gs.group_size = 0;

            // Calculate group's key range and total size
            InternalKey group_smallest = gs.files[0]->smallest;
            InternalKey group_largest = gs.files[0]->largest;

            for (auto* f : gs.files) {
              gs.group_size += f->fd.GetFileSize();
              if (icmp->Compare(f->smallest, group_smallest) < 0) {
                group_smallest = f->smallest;
              }
              if (icmp->Compare(f->largest, group_largest) > 0) {
                group_largest = f->largest;
              }
            }

            // Calculate L1 overlap size
            std::vector<FileMetaData*> l1_overlaps;
            vstorage_->GetOverlappingInputs(output_level_, &group_smallest,
                                             &group_largest, &l1_overlaps);
            gs.l1_overlap_size = 0;
            for (auto* f : l1_overlaps) {
              gs.l1_overlap_size += f->fd.GetFileSize();
            }

            // Calculate overlap ratio (L1 overlap / group size)
            gs.overlap_ratio = (gs.group_size > 0) ?
                static_cast<double>(gs.l1_overlap_size) / static_cast<double>(gs.group_size) : 0.0;

            scored_groups.push_back(std::move(gs));
          }

          if (!scored_groups.empty()) {
            // Find N_max and R_max for normalization
            size_t n_max = 0;
            double r_max = 0.0;
            for (const auto& gs : scored_groups) {
              if (gs.file_count > n_max) n_max = gs.file_count;
              if (gs.overlap_ratio > r_max) r_max = gs.overlap_ratio;
            }

            // Calculate normalized score for each group
            double alpha = ioptions_.l0_file_count_weight;
            for (auto& gs : scored_groups) {
              double normalized_file_count = (n_max > 0) ?
                  static_cast<double>(gs.file_count) / static_cast<double>(n_max) : 0.0;
              double normalized_overlap_efficiency = (r_max > 0) ?
                  (1.0 - gs.overlap_ratio / r_max) : 1.0;

              gs.score = alpha * normalized_file_count +
                         (1.0 - alpha) * normalized_overlap_efficiency;
            }

            // Sort by score (highest first), use file_count as tie-breaker
            std::sort(scored_groups.begin(), scored_groups.end(),
            ...
            ROCKS_LOG_INFO(ioptions_.logger,
                           "[%s] L0 weighted group selection: picked group with "
                           "%zu files, score=%.4f (file_count_norm=%.4f, overlap_ratio=%.4f, "
                           "group_size=%" PRIu64 ", l1_overlap=%" PRIu64 ")",
                           cf_name_.c_str(),
                           scored_groups[0].file_count,
                           scored_groups[0].score,
                           (n_max > 0) ? static_cast<double>(scored_groups[0].file_count) / n_max : 0.0,
                           scored_groups[0].overlap_ratio,
                           scored_groups[0].group_size,
                           scored_groups[0].l1_overlap_size);

            start_level_inputs_.files = scored_groups[0].files;
            start_level_inputs_.level = 0;
            return true;
          }
        } else {
          //jw: Original logic - sort groups by file count (largest first)
          std::vector<std::vector<FileMetaData*>> sorted_groups;
          for (auto& kv : groups) {
            sorted_groups.push_back(std::move(kv.second));
          }
          std::sort(sorted_groups.begin(), sorted_groups.end(),
                    [](const std::vector<FileMetaData*>& a, const std::vector<FileMetaData*>& b) {
                      return a.size() > b.size();
                    });

          // All groups are guaranteed to have no being_compacted files
          // (we filtered them out at the beginning)
          if (!sorted_groups.empty()) {
            start_level_inputs_.files = sorted_groups[0];
            start_level_inputs_.level = 0;
            return true;
          }
        }
...
bool LevelCompactionBuilder::PickIntraL0Compaction() {
  //jw: Check if intra-L0 compaction is enabled
  if (!ioptions_.enable_intra_l0_compaction) {
    return false;
  }
  ...
  //jw: When use_new_flush is enabled (1=split, 2=bucket), use overlapping-group-based L0-L0 compaction
  //    This preserves the key range structure created by the new flush logic
  if (ioptions_.use_new_flush != 0) {
    // Find all overlapping groups in L0
    auto groups = vstorage_->FindL0OverlappingGroups(
        ioptions_.user_comparator);

    // Try to find a suitable group for compaction
    for (const auto& group : groups) {
      // Skip groups with too few files
      if (group.files.size() < static_cast<size_t>(kMinFilesForIntraL0Compaction)) {
        continue;
      }
      ...
      // Check if group size is within max_compaction_bytes
      if (total_size <= mutable_cf_options_.max_compaction_bytes) {
        // Found a suitable group!
        start_level_inputs_.level = 0;
        start_level_inputs_.files = group.files;

        ROCKS_LOG_INFO(ioptions_.logger,
                       "[%s] Picked L0-L0 intra compaction (new_flush mode): "
                       "%zu files, total size %" PRIu64 " bytes",
                       cf_name_.c_str(), group.files.size(), total_size);
        return true;
      }
    }
  ...
  //jw: Original RocksDB behavior when use_new_flush=0
  //    This merges consecutive L0 files from newest without key range consideration
  /*
  return FindIntraL0Compaction(level_files, kMinFilesForIntraL0Compaction,
                               ...
                               &start_level_inputs_);
  */
  //jw: Disable original L0-L0 compaction when use_new_flush=0
...
bool LevelCompactionBuilder::PickSizeBasedIntraL0Compaction() {
  ...
  //jw: When use_new_flush is enabled (1=split, 2=bucket), use overlapping-group-based approach
  if (ioptions_.use_new_flush != 0) {
    auto groups = vstorage_->FindL0OverlappingGroups(
        ioptions_.user_comparator);

    for (const auto& group : groups) {
      if (group.files.size() < min_num_file) {
        continue;
      }
      ...
      if (total_size <= mutable_cf_options_.max_compaction_bytes) {
        ...
        ROCKS_LOG_INFO(ioptions_.logger,
                       "[%s] Picked size-based L0-L0 intra compaction: "
                       "%zu files, total size %" PRIu64 " bytes",
                       cf_name_.c_str(), group.files.size(), total_size);
        return true;
      }
  ...
  //jw: Original behavior when use_new_flush=0
  /*
  start_level_inputs_.clear();
  start_level_inputs_.level = 0;
  for (const auto& file : l0_files) {
  ...
  return true;
  */
  //jw: Disable when use_new_flush=0
  return false;
}

//jw: Detect L0 group boundaries using Union-Find algorithm
//    Exclude being_compacted files to get boundaries for NEW L1 files
std::vector<Compaction::L0GroupBoundary> LevelCompactionBuilder::DetectL0GroupBoundaries() {
  std::vector<Compaction::L0GroupBoundary> boundaries;
  ...
  //jw: Track which files are being compacted (exclude from group calculation)
  std::vector<bool> is_being_compacted(num_files, false);
  int num_active_files = 0;
  for (size_t i = 0; i < num_files; ++i) {
    if (l0_files[i]->being_compacted) {
      is_being_compacted[i] = true;
    } else {
      num_active_files++;
    }
  }

  // If all files are being compacted or less than 2 active files, no boundaries
  ...
  // Union-Find helper functions (use std::function for recursion)
  std::function<int(int)> find = [&parent, &find](int x) -> int {
    if (parent[x] != x) {
  ...
  //jw: Check overlap and union overlapping files (excluding being_compacted)
  //jw: Use User Key comparison for overlap detection (consistency with GetOverlappingInputs())
  //jw: Use InternalKey comparison for min/max key calculation (consistency with GetRange())
  const InternalKeyComparator* icmp = vstorage_->InternalComparator();
  const Comparator* ucmp = icmp->user_comparator();
  for (size_t i = 0; i < num_files; ++i) {
    if (is_being_compacted[i]) continue;  //jw: Skip files being compacted

    for (size_t j = i + 1; j < num_files; ++j) {
      if (is_being_compacted[j]) continue;  //jw: Skip files being compacted

      //jw: User Key overlap check: NOT (i_end < j_start OR j_end < i_start)
      Slice i_end = l0_files[i]->largest.user_key();
      Slice j_start = l0_files[j]->smallest.user_key();
      Slice j_end = l0_files[j]->largest.user_key();
      Slice i_start = l0_files[i]->smallest.user_key();
  ...
  //jw: Group files by their root parent (excluding being_compacted files)
  ...
  // For each group, find the min_key and max_key
  struct GroupInfo {
    InternalKey min_key;
    InternalKey max_key;

    GroupInfo(const InternalKey& min, const InternalKey& max)
  ...
  for (const auto& kv : groups) {
    ...
    InternalKey group_min = l0_files[group[0]]->smallest;
    InternalKey group_max = l0_files[group[0]]->largest;

    for (size_t i = 1; i < group.size(); ++i) {
      if (icmp->Compare(l0_files[group[i]]->smallest, group_min) < 0) {
        group_min = l0_files[group[i]]->smallest;
      }
      if (icmp->Compare(l0_files[group[i]]->largest, group_max) > 0) {
        group_max = l0_files[group[i]]->largest;
      }
    }

    group_infos.emplace_back(group_min, group_max);
  }

  // Sort groups by min_key
  std::sort(group_infos.begin(), group_infos.end(),
            [icmp](const GroupInfo& a, const GroupInfo& b) {
              return icmp->Compare(a.min_key, b.min_key) < 0;
            });

  //jw: Extract boundaries for each group (store both min_key and max_key)
  //    Strategy: Store [min_key, max_key] for each L0 group
  //    This enables better L1 file alignment for trivial move optimization
  //
  //    Example:
  //      L0 Groups: [a-e] [g-k] [m-z]
  //      Boundaries: [{a,e}, {g,k}, {m,z}]
  //
  //    L1 splitting logic (in compaction_outputs.cc):
  //      - Split at group max_key when file size is 50-100% of target
  //      - Check if next key would belong to next group (using min_key)
  //      - This prevents gap keys from creating misalignment
  for (const auto& group_info : group_infos) {
    boundaries.emplace_back(
        group_info.min_key.user_key().ToString(),
        group_info.max_key.user_key().ToString());
  }
