...
void DBImpl::MaybeScheduleFlushOrCompaction() {
  ...
  // auto fqarg=flush_queue_.front();
  // for (const auto& [cfd, max_memtable_id] :
  //   fqarg.cfd_to_max_mem_id_to_persist) {

  //     l0stall=cfd->current()->storage_info()->l0_delay_trigger_count()/
  //     cfd->GetLatestMutableCFOptions().level0_stop_writes_trigger;
  // }
  // if(immutable_db_options_.use_new_flush!=0&&l0stall){
  //   bg_job_limits.max_flushes=1;
  // }
...
ColumnFamilyData* DBImpl::PickCompactionFromQueue(
  ...
  while (!compaction_queue_.empty()) {
    ...
    compaction_queue_.pop_front();
    // For parallel L0 compaction (use_new_flush=1), multiple compaction jobs
    // from the same CFD can be enqueued, so queued_for_compaction flag
    // may already be false for subsequent jobs
    ...
    cfd = first_cfd;
    // Only clear the flag if it's set (for the first job from this CFD)
...
/**/
//jw: Re-enable parallel L0 compaction logic to handle multiple groups
void DBImpl::EnqueuePendingCompaction(ColumnFamilyData* cfd) {
  mutex_.AssertHeld();
  if (reject_new_background_jobs_) {
    return;
  }
  //jw: Check if we should use parallel L0 compaction or regular compaction
  if (!cfd->queued_for_compaction()) {
    int num_groups = cfd->GetNumGroupsNeedingCompaction();

    // If GetNumGroupsNeedingCompaction returns 0 but NeedsCompaction is true,
    // it means we're not using parallel L0 compaction (use_new_flush=false)
    if (num_groups == 0 && cfd->NeedsCompaction()) {
      // Use original logic for non-parallel case
      TEST_SYNC_POINT_CALLBACK("EnqueuePendingCompaction::cfd",
                               static_cast<void*>(cfd));
      AddToCompactionQueue(cfd);
    } else if (num_groups > 0) {
      // For parallel L0 compaction, enqueue multiple compaction jobs for each group
      TEST_SYNC_POINT_CALLBACK("EnqueuePendingCompaction::cfd",
                               static_cast<void*>(cfd));
      // Add multiple entries to the compaction queue (one for each group)
      for (int i = 0; i < num_groups; ++i) {
        if (i == 0) {
          // First one uses the normal AddToCompactionQueue (sets queued flag)
          AddToCompactionQueue(cfd);
        } else {
          // Additional ones: manually increment unscheduled_compactions_
          // without setting queued flag (to allow multiple from same CFD)
          cfd->Ref();
          compaction_queue_.push_back(cfd);
          ++unscheduled_compactions_;
        }
  ...
  *reason = FlushReason::kOthers;
  ...
  while (!flush_queue_.empty()) {
    ...
    }
    {
      ColumnFamilyData* cfd =
      flush_req.cfd_to_max_mem_id_to_persist.begin()->first;
      unsigned int l0stall=cfd->current()->storage_info()->l0_delay_trigger_count()/
      cfd->GetLatestMutableCFOptions().level0_stop_writes_trigger;
      if(immutable_db_options_.use_new_flush!=0&&l0stall){
        // bg_job_limits.max_flushes=1;
        EnqueuePendingFlush(flush_req);
  ...
  *made_progress = false;
  ...
  if (is_manual) {
    ...
    if (!mutable_cf_options.disable_auto_compactions && !cfd->IsDropped()) {
      ...
      if (c != nullptr) {
        //jw: Register pending compaction files for intra-L0 compaction (only with use_new_flush=1 or 2)
        // This prevents concurrent flushes from incorrectly splitting when this compaction's
        // output files are not yet reflected in the version
        bool is_intra_l0 = (c->start_level() == 0 && c->output_level() == 0);
        if (is_intra_l0 && cfd->ioptions().use_new_flush != 0) {
          // Estimate output range from input files
          std::vector<std::pair<std::string, std::string>> estimated_ranges;
          const auto* inputs = c->inputs(0);
          if (inputs && !inputs->empty()) {
            // For intra-L0, estimate the output range as the union of all input ranges
            std::string smallest = (*inputs)[0]->smallest.user_key().ToString();
            std::string largest = (*inputs)[0]->largest.user_key().ToString();

            for (size_t i = 1; i < inputs->size(); ++i) {
              const auto* file = (*inputs)[i];
              const auto& icmp = c->column_family_data()->internal_comparator();
              if (icmp.user_comparator()->Compare(file->smallest.user_key(), smallest) < 0) {
                smallest = file->smallest.user_key().ToString();
              }
              if (icmp.user_comparator()->Compare(file->largest.user_key(), largest) > 0) {
                largest = file->largest.user_key().ToString();
              }
            }

            estimated_ranges.emplace_back(smallest, largest);
            cfd->RegisterPendingCompactionFiles(job_context->job_id, estimated_ranges);

            ROCKS_LOG_BUFFER(log_buffer,
                           "[%s] [JOB %d] Registered pending intra-L0 compaction range: [%s, %s]\n",
                           cfd->GetName().c_str(), job_context->job_id,
                           smallest.c_str(), largest.c_str());
          }
        }
        ...
        if (!enough_room) {
          //jw: Unregister if compaction is cancelled (only with use_new_flush=1 or 2)
          if (is_intra_l0 && cfd->ioptions().use_new_flush != 0) {
            cfd->UnregisterPendingCompactionFiles(job_context->job_id);
            ROCKS_LOG_BUFFER(log_buffer,
                           "[%s] [JOB %d] Unregistered pending compaction (cancelled - not enough room)\n",
                           cfd->GetName().c_str(), job_context->job_id);
          }
  ...
  //jw: Unregister pending compaction files (only with use_new_flush=1 or 2)
  // This must be done before c.reset() so we can still access compaction info
  if (c != nullptr && c->start_level() == 0 && c->output_level() == 0 &&
      c->column_family_data()->ioptions().use_new_flush != 0) {
    c->column_family_data()->UnregisterPendingCompactionFiles(job_context->job_id);
    ROCKS_LOG_BUFFER(log_buffer,
                   "[%s] [JOB %d] Unregistered pending intra-L0 compaction files\n",
