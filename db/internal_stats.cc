...
void InternalStats::DumpCFStatsWriteStall(std::string* value,
  ...
  // Add write stall time statistics
  str << "Write Stall (time in seconds): ";
  char time_buf[64];
  snprintf(time_buf, sizeof(time_buf), "%.3f",
           cf_stats_value_[MEMTABLE_LIMIT_DELAYS_MICROS] / 1000000.0);
  str << "memtable-limit-delays: " << time_buf << ", ";
  snprintf(time_buf, sizeof(time_buf), "%.3f",
           cf_stats_value_[MEMTABLE_LIMIT_STOPS_MICROS] / 1000000.0);
  str << "memtable-limit-stops: " << time_buf << ", ";
  snprintf(time_buf, sizeof(time_buf), "%.3f",
           cf_stats_value_[L0_FILE_COUNT_LIMIT_DELAYS_MICROS] / 1000000.0);
  str << "l0-file-count-limit-delays: " << time_buf << ", ";
  snprintf(time_buf, sizeof(time_buf), "%.3f",
           cf_stats_value_[L0_FILE_COUNT_LIMIT_STOPS_MICROS] / 1000000.0);
  str << "l0-file-count-limit-stops: " << time_buf << ", ";
  snprintf(time_buf, sizeof(time_buf), "%.3f",
           cf_stats_value_[PENDING_COMPACTION_BYTES_LIMIT_DELAYS_MICROS] / 1000000.0);
  str << "pending-compaction-bytes-delays: " << time_buf << ", ";
  snprintf(time_buf, sizeof(time_buf), "%.3f",
           cf_stats_value_[PENDING_COMPACTION_BYTES_LIMIT_STOPS_MICROS] / 1000000.0);
  str << "pending-compaction-bytes-stops: " << time_buf << "\n";
