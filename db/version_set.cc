...
//jw: L0 Range-Aware Scan Iterator
// This iterator lazily creates iterators for L0 SST files based on key range overlap.
// Only files whose key range overlaps with the current scan position will have
// iterators created, reducing unnecessary I/O for L0 scans.
class L0RangeScanIterator final : public InternalIterator {
 public:
  // L0 file metadata for lazy iterator creation
  struct L0FileInfo {
    FileMetaData* file_metadata;
    Slice smallest_key;  // internal key
    Slice largest_key;   // internal key
    bool iterator_active = false;  // true if iterator is in the heap
  };
  ...
  L0RangeScanIterator(
      ...
      uint64_t max_file_size_for_l0_meta_pin)
    ...
    // Initialize L0 file info
    for (size_t i = 0; i < flevel_->num_files; i++) {
      const auto& file = flevel_->files[i];
      L0FileInfo info;
      info.file_metadata = file.file_metadata;
      info.smallest_key = file.smallest_key;
      info.largest_key = file.largest_key;
      info.iterator_active = false;
      l0_files_.push_back(info);
    }
  }

  ~L0RangeScanIterator() override {
    ClearAllIterators();
  }
  ...
  void Seek(const Slice& target) override {
    ...
    // Find files that overlap with target
    AddOverlappingFiles(target);

    // Seek all active iterators
    for (auto& iter_info : active_iterators_) {
      iter_info.iter->Seek(target);
    }
  ...
  void SeekForPrev(const Slice& target) override {
    ...
    // Find files that overlap with target
    AddOverlappingFilesForPrev(target);

    // SeekForPrev all active iterators
    for (auto& iter_info : active_iterators_) {
      iter_info.iter->SeekForPrev(target);
    }
  ...
  void SeekToFirst() override {
    ...
    // Activate all files for SeekToFirst
    for (size_t i = 0; i < l0_files_.size(); i++) {
      ActivateFile(i);
    }

    // SeekToFirst on all iterators
    for (auto& iter_info : active_iterators_) {
      iter_info.iter->SeekToFirst();
    }
  ...
  void SeekToLast() override {
    ...
    // Activate all files for SeekToLast
    for (size_t i = 0; i < l0_files_.size(); i++) {
      ActivateFile(i);
    }

    // SeekToLast on all iterators
    for (auto& iter_info : active_iterators_) {
      iter_info.iter->SeekToLast();
    }
  ...
  void Next() override {
    assert(Valid());
    assert(current_index_ < active_iterators_.size());

    // Remember the last key before moving (needed to find next file if all exhausted)
    std::string last_key = key().ToString();

    // Move the current (smallest) iterator forward
    active_iterators_[current_index_].iter->Next();

    // Find new smallest
    FindSmallest();

    if (Valid()) {
      // Check if current key has entered a new file's range
      MaybeAddMoreFilesForward(key());
    } else {
      // All active iterators are exhausted
      // Try to find and activate the next file whose range starts after last_key
      TryActivateNextFile(Slice(last_key));
    }
  }
  ...
  void Prev() override {
    ...
    // Move the current (largest) iterator backward
    active_iterators_[current_index_].iter->Prev();
  ...
  bool Valid() const override {
    return current_index_ < active_iterators_.size() &&
           active_iterators_[current_index_].iter->Valid();
  }

  Slice key() const override {
    assert(Valid());
    return active_iterators_[current_index_].iter->key();
  }

  Slice value() const override {
    assert(Valid());
    return active_iterators_[current_index_].iter->value();
  }
  ...
  Status status() const override {
    ...
    for (const auto& iter_info : active_iterators_) {
      if (!iter_info.iter->status().ok()) {
        return iter_info.iter->status();
      }
    }
    return Status::OK();
  }
  ...
  bool PrepareValue() override {
    assert(Valid());
    return active_iterators_[current_index_].iter->PrepareValue();
  }

  bool IsKeyPinned() const override {
    return Valid() && active_iterators_[current_index_].iter->IsKeyPinned();
  }

  bool IsValuePinned() const override {
    return Valid() && active_iterators_[current_index_].iter->IsValuePinned();
  }

  void SetPinnedItersMgr(PinnedIteratorsManager* pinned_iters_mgr) override {
    pinned_iters_mgr_ = pinned_iters_mgr;
    for (auto& iter_info : active_iterators_) {
      iter_info.iter->SetPinnedItersMgr(pinned_iters_mgr);
    }
  }
 ...
 private:
  ...
  void ClearAllIterators() {
    for (auto& iter_info : active_iterators_) {
      delete iter_info.iter;
    }
    active_iterators_.clear();
    for (auto& file_info : l0_files_) {
      file_info.iterator_active = false;
    }
    current_index_ = 0;
  }

  // Create iterator for a file and add to active list
  void ActivateFile(size_t file_index) {
    if (l0_files_[file_index].iterator_active) {
      return;  // Already active
    }

    auto* file_meta = l0_files_[file_index].file_metadata;
    InternalIterator* iter = table_cache_->NewIterator(
        read_options_, file_options_, icomparator_,
        *file_meta, /*range_del_agg=*/nullptr, mutable_cf_options_,
        nullptr, file_read_hist_, caller_,
        /*arena=*/nullptr, skip_filters_, /*level=*/0,
    ...
    ActiveIterator active_iter;
    active_iter.iter = iter;
    active_iter.file_index = file_index;
    active_iterators_.push_back(active_iter);

    l0_files_[file_index].iterator_active = true;
  }

  // Add files that overlap with target key (for forward scan)
  // Only activate files where target falls within [smallest, largest]
  // Files where smallest > target will be activated later by MaybeAddMoreFilesForward()
  // when the scan reaches their range
  void AddOverlappingFiles(const Slice& target) {
    for (size_t i = 0; i < l0_files_.size(); i++) {
      const auto& file_info = l0_files_[i];
      // Activate file if: smallest_key <= target <= largest_key
      // This means target is within the file's key range
      if (icomparator_.Compare(file_info.smallest_key, target) <= 0 &&
          icomparator_.Compare(file_info.largest_key, target) >= 0) {
        ActivateFile(i);
      }
    }
  }

  // Add files that overlap for SeekForPrev (backward scan)
  // Only activate files where target falls within [smallest, largest]
  void AddOverlappingFilesForPrev(const Slice& target) {
    for (size_t i = 0; i < l0_files_.size(); i++) {
      const auto& file_info = l0_files_[i];
      // Activate file if: smallest_key <= target <= largest_key
      if (icomparator_.Compare(file_info.smallest_key, target) <= 0 &&
  ...
  // Called after Next() to check if we need to add more files for forward scan
  // A file needs to be added if:
  // - It's not yet active
  // - current_key >= file.smallest_key (we've entered or passed the file's range)
  // - current_key <= file.largest_key (we haven't passed the file's range yet)
  void MaybeAddMoreFilesForward(const Slice& current_key) {
    bool added_new_file = false;
    for (size_t i = 0; i < l0_files_.size(); i++) {
      if (l0_files_[i].iterator_active) {
        continue;  // Already active
      }
      const auto& file_info = l0_files_[i];
      // Check if current_key is within this file's range
      // smallest_key <= current_key <= largest_key
      if (icomparator_.Compare(file_info.smallest_key, current_key) <= 0 &&
          icomparator_.Compare(file_info.largest_key, current_key) >= 0) {
        ActivateFile(i);
        // Seek to current_key - this file might have the same key or smaller keys
        // that we need to consider
        active_iterators_.back().iter->Seek(current_key);
        added_new_file = true;
      }
    }
    // If we added new files, we need to re-find the smallest
    // The new file might have a key smaller than current_key
    if (added_new_file) {
      FindSmallest();
    }
  }

  // Called when all active iterators are exhausted after Next()
  // Find the next file whose smallest_key > last_key and activate it
  void TryActivateNextFile(const Slice& last_key) {
    // Find the file with the smallest smallest_key that is > last_key
    size_t best_file = l0_files_.size();  // Invalid
    for (size_t i = 0; i < l0_files_.size(); i++) {
      if (l0_files_[i].iterator_active) {
        continue;  // Already active (and exhausted)
      }
      const auto& file_info = l0_files_[i];
      // File's smallest_key must be > last_key (next range)
      if (icomparator_.Compare(file_info.smallest_key, last_key) > 0) {
        if (best_file >= l0_files_.size() ||
            icomparator_.Compare(file_info.smallest_key,
                                 l0_files_[best_file].smallest_key) < 0) {
          best_file = i;
        }
      }
    }
    ...
    if (best_file < l0_files_.size()) {
      // Found a next file, activate it and seek to first
      ActivateFile(best_file);
      active_iterators_.back().iter->SeekToFirst();
      FindSmallest();

      // There might be other files overlapping with this new position
      if (Valid()) {
        MaybeAddMoreFilesForward(key());
      }
    }
    // If no next file found, iterator becomes invalid (end of L0 scan)
  }

  // Find the iterator with the smallest key
  void FindSmallest() {
    current_index_ = active_iterators_.size();  // Invalid

    for (size_t i = 0; i < active_iterators_.size(); i++) {
      if (active_iterators_[i].iter->Valid()) {
        if (current_index_ >= active_iterators_.size() ||
            icomparator_.Compare(active_iterators_[i].iter->key(),
                                 active_iterators_[current_index_].iter->key()) < 0) {
          current_index_ = i;
        }
      }
    }
  ...
  uint64_t max_file_size_for_l0_meta_pin_;

  std::vector<L0FileInfo> l0_files_;
  std::vector<ActiveIterator> active_iterators_;
...
void Version::AddIteratorsForLevel(const ReadOptions& read_options,
  ...
  auto* arena = merge_iter_builder->GetArena();
  if (level == 0) {
    //jw: Check if new_range_scan is enabled
    const auto& db_options = cfd_->ioptions();
    if (db_options.new_range_scan) {
      //jw: Use L0RangeScanIterator for lazy, range-aware L0 scanning
      auto* mem = arena->AllocateAligned(sizeof(L0RangeScanIterator));
      auto l0_iter = new (mem) L0RangeScanIterator(
          cfd_->table_cache(), read_options, soptions,
          cfd_->internal_comparator(), &storage_info_.LevelFilesBrief(0),
          mutable_cf_options_,
          cfd_->internal_stats()->GetFileReadHist(0),
          TableReaderCaller::kUserIterator,
          /*skip_filters=*/false, allow_unprepared_value,
          max_file_size_for_l0_meta_pin_);
      merge_iter_builder->AddIterator(l0_iter);
    } else {
      // Original: Merge all level zero files together since they may overlap
...
//jw: Helper function for group-based L0 pending bytes estimation
//    This is used when use_new_flush is enabled (split-flush design)
//    Only counts groups with >= 4 files as "urgent" compaction work
uint64_t VersionStorageInfo::EstimateL0CompactionBytesNeededByGroups(
    const MutableCFOptions& mutable_cf_options) {
  (void)mutable_cf_options;  // Suppress unused parameter warning

  // Find overlapping groups in L0
  auto groups = FindL0OverlappingGroups(user_comparator_);

  if (groups.empty()) {
    // Fallback: use total L0 size
    uint64_t total_l0_size = 0;
    for (auto* f : files_[0]) {
      total_l0_size += f->fd.GetFileSize();
    }
    return total_l0_size;
  }

  // Sum up L0 file sizes in groups with >= 4 files (urgent groups)
  const int kMinFilesPerGroup = 4;
  uint64_t effective_l0_size = 0;

  for (const auto& group : groups) {
    if (static_cast<int>(group.files.size()) >= kMinFilesPerGroup) {
      for (auto* file : group.files) {
        effective_l0_size += file->fd.GetFileSize();
      }
    }
  }

  // If no group has >= 4 files, fall back to total L0 size
  if (effective_l0_size == 0) {
    for (auto* f : files_[0]) {
      effective_l0_size += f->fd.GetFileSize();
    }
  }

  // Calculate L1 size for ratio-based estimation
  uint64_t l1_size = 0;
  int base_lvl = base_level();
  if (base_lvl < num_levels_) {
    for (auto* f : files_[base_lvl]) {
      l1_size += f->fd.GetFileSize();
    }
  }

  // Calculate total L0 size for ratio calculation
  uint64_t total_l0_size = 0;
  for (auto* f : files_[0]) {
    total_l0_size += f->fd.GetFileSize();
  }

  // Estimate L1 contribution using size ratio (similar to other levels)
  // ratio = L1_size / L0_total_size
  double l1_ratio = (total_l0_size > 0 && l1_size > 0)
                    ? static_cast<double>(l1_size) / static_cast<double>(total_l0_size)
                    : 0.0;

  // Total pending bytes = effective_l0_size * (1 + l1_ratio)
  // This matches the fanout calculation style used in other levels
  return static_cast<uint64_t>(
      static_cast<double>(effective_l0_size) * (1.0 + l1_ratio));
}

void VersionStorageInfo::EstimateCompactionBytesNeeded(
    const MutableCFOptions& mutable_cf_options,
    const ImmutableOptions& immutable_options) { //jw: Added parameter
  ...
  if (static_cast<int>(files_[0].size()) >=
    ...
    //jw: Use group-based estimation when use_new_flush is enabled (1=split, 2=bucket)
    //    This better reflects actual compaction work for split-flush design
    if (immutable_options.use_new_flush != 0) {
      // Group-based calculation: only counts groups with >= 4 files
      // and uses ratio-based L1 estimation (similar to other levels)
      estimated_compaction_needed_bytes_ =
          EstimateL0CompactionBytesNeededByGroups(mutable_cf_options);
      ROCKS_LOG_INFO(
          immutable_options.logger,
          "[EstimateCompactionBytes] L0 triggered (use_new_flush=1): "
          "L0 files=%zu, L0 size=%" PRIu64 ", estimated pending bytes=%" PRIu64,
          files_[0].size(), level_size, estimated_compaction_needed_bytes_);
    } else {
      // Original calculation for baseline
      estimated_compaction_needed_bytes_ = level_size;
      ROCKS_LOG_INFO(
          immutable_options.logger,
          "[EstimateCompactionBytes] L0 triggered (use_new_flush=0): "
          "L0 files=%zu, L0 size=%" PRIu64 ", pending bytes=%" PRIu64,
          files_[0].size(), level_size, estimated_compaction_needed_bytes_);
    }
  ...
  for (int level = base_level(); level <= MaxInputLevel(); level++) {
    ...
    //jw: For baseline, add L1 size when L0 compaction is triggered
    //    For use_new_flush (1=split, 2=bucket), L1 contribution is already included in group-based calc
    uint64_t prev_estimated = estimated_compaction_needed_bytes_;
    if (level == base_level() && level0_compact_triggered &&
        immutable_options.use_new_flush == 0) {
      // Add base level size to compaction if level0 compaction triggered.
      estimated_compaction_needed_bytes_ += level_size;
      ROCKS_LOG_INFO(
          immutable_options.logger,
          "[EstimateCompactionBytes] L%d (base level) contribution: "
          "level_size=%" PRIu64 ", total pending bytes=%" PRIu64,
          level, level_size, estimated_compaction_needed_bytes_);
    }
    ...
    if (level_size > level_target) {
      ...
      if (bytes_next_level > 0) {
        assert(level_size > 0);
        uint64_t level_contribution = static_cast<uint64_t>(
            static_cast<double>(bytes_compact_to_next_level) *
            (static_cast<double>(bytes_next_level) /
                 static_cast<double>(level_size) +
             1));
        estimated_compaction_needed_bytes_ += level_contribution;
        ROCKS_LOG_INFO(
            immutable_options.logger,
            "[EstimateCompactionBytes] L%d exceeds target: "
            "level_size=%" PRIu64 ", target=%" PRIu64 ", excess=%" PRIu64
            ", L%d size=%" PRIu64 ", contribution=%" PRIu64
            ", total pending bytes=%" PRIu64,
            level, level_size, level_target, bytes_compact_to_next_level,
            level + 1, bytes_next_level, level_contribution,
            estimated_compaction_needed_bytes_);
      } else {
        ROCKS_LOG_INFO(
            immutable_options.logger,
            "[EstimateCompactionBytes] L%d exceeds target but L%d is empty: "
            "level_size=%" PRIu64 ", target=%" PRIu64 ", excess=%" PRIu64,
            level, level + 1, level_size, level_target, bytes_compact_to_next_level);
      }
    } else if (estimated_compaction_needed_bytes_ > prev_estimated) {
      // Log when this level contributed to pending bytes
      ROCKS_LOG_INFO(
          immutable_options.logger,
          "[EstimateCompactionBytes] L%d contribution: "
          "added=%" PRIu64 ", total pending bytes=%" PRIu64,
          level, estimated_compaction_needed_bytes_ - prev_estimated,
          estimated_compaction_needed_bytes_);
    }
  }
  ...
  ROCKS_LOG_INFO(
      immutable_options.logger,
      "[EstimateCompactionBytes] Final: total pending bytes=%" PRIu64
      " (soft limit: %" PRIu64 ", hard limit: %" PRIu64 ")",
      estimated_compaction_needed_bytes_,
...
void VersionStorageInfo::ComputeCompactionScore(
  ...
  unsigned int l0stall =0;
  unsigned int pendingstall=(estimated_compaction_needed_bytes_/mutable_cf_options.hard_pending_compaction_bytes_limit);
    ...
    if (level == 0) {
      ...
      if (compaction_style_ == kCompactionStyleFIFO) {
        ...
        if (score < 1 &&
          ...
          //jw: Use l0_delay_trigger_count when use_new_flush is enabled (1=split, 2=bucket)
          double effective_l0_count = (immutable_options.use_new_flush != 0)
              ? static_cast<double>(l0_delay_trigger_count_)
              : static_cast<double>(num_sorted_runs);
              //jw: L0 compaction trigger condition
          score = std::max(
              effective_l0_count / mutable_cf_options.level0_file_num_compaction_trigger,
              score);
        }
        ...
        //jw: Use l0_delay_trigger_count (which can be group-based) instead of num_sorted_runs
        // when use_new_flush is enabled (1=split, 2=bucket)
        double effective_l0_count = (immutable_options.use_new_flush != 0)
            ? static_cast<double>(l0_delay_trigger_count_)
            : static_cast<double>(num_sorted_runs);
          // if(immutable_options.use_new_flush!=0){
          l0stall = (effective_l0_count/mutable_cf_options.level0_stop_writes_trigger);
          // }
          score = effective_l0_count / mutable_cf_options.level0_file_num_compaction_trigger;
        //jw: DISABLED size-based L0 compaction scoring for use_new_flush=0
        //    Only use file count-based triggering (effective_l0_count / level0_file_num_compaction_trigger)
        //    to maintain consistent compaction behavior based purely on file count
        /*
        if (compaction_style_ == kCompactionStyleLevel && num_levels() > 1) {
          ...
          if (immutable_options.level_compaction_dynamic_level_bytes) {
            ...
            //jw: Only apply this scoring when use_new_flush is disabled (original mode)
            if (immutable_options.use_new_flush == 0) {
              score = std::max(score,
  ...
  if(immutable_options.use_new_flush!=0){
    if(l0stall && !pendingstall){
      compaction_score_[0] = 20180546.0;
    }else if(!l0stall && pendingstall){
      for (int level = 1; level <= MaxInputLevel(); level++) {
        if(compaction_score_[level]>1.0){
          compaction_score_[level]*=(level);
        }
      }
    }
  }
  ...
  EstimateCompactionBytesNeeded(mutable_cf_options, immutable_options);
}
...
//jw
// Calculate the maximum overlapping depth in L0 using Sweep Line algorithm
// This is used for slowdown/stop triggers (NOT for compaction trigger)
// Returns the maximum number of files that overlap at any single point in key space
// This accurately represents worst-case read latency (max files to check for a key)
// Time complexity: O(N log N) - faster than previous Union-Find O(N²)
int VersionStorageInfo::CalculateMaxOverlappingGroupSize() const {
  const auto& l0_files = files_[0];
  int num_files = static_cast<int>(l0_files.size());
  ...
  if (num_files == 1) {
    return l0_files[0]->being_compacted ? 0 : 1;
  }

  //jw: Use Sweep Line algorithm for exact max overlap calculation
  //    Event: (key, type, file_number)
  //    type: 0 = START, 1 = END
  struct Event {
    Slice key;
    ...
    Event(const Slice& k, int t, int idx) : key(k), type(t), file_idx(idx) {}
  };

  std::vector<Event> events;
  events.reserve(num_files * 2);

  // Step 1: Create events for each non-compacting file (O(N))
  int num_active_files = 0;
  for (int i = 0; i < num_files; i++) {
    if (l0_files[i]->being_compacted) continue;

    num_active_files++;
    Slice start_key = ExtractUserKey(l0_files[i]->smallest.Encode());
    Slice end_key = ExtractUserKey(l0_files[i]->largest.Encode());

    events.emplace_back(start_key, 0, i);  // START event
    events.emplace_back(end_key, 1, i);    // END event
  }
  ...
  // Step 2: Sort events by key (O(N log N))
  const Comparator* ucmp = internal_comparator_->user_comparator();
  std::sort(events.begin(), events.end(),
    [ucmp](const Event& a, const Event& b) {
      int cmp = ucmp->Compare(a.key, b.key);
      if (cmp != 0) return cmp < 0;
      // At same key: START events (0) before END events (1)
      // This ensures we count correctly when ranges touch at boundaries
      return a.type < b.type;
    });

  // Step 3: Sweep through events and track max depth (O(N))
  int current_depth = 0;
  int max_depth = 0;

  for (const auto& event : events) {
    if (event.type == 0) {  // START
      current_depth++;
...
//jw
// Get all files in the largest overlapping group in L0
std::vector<FileMetaData*> VersionStorageInfo::GetLargestOverlappingGroup() const {
  const auto& l0_files = files_[0];
  int num_files = static_cast<int>(l0_files.size());
  ...
  std::vector<int> rank(num_files, 0);

  // Initialize: each file is its own group
  for (int i = 0; i < num_files; i++) {
    parent[i] = i;
  }
  ...
  // Check overlap between all pairs of files
  for (int i = 0; i < num_files; i++) {
    for (int j = i + 1; j < num_files; j++) {
      const auto* file_i = l0_files[i];
      const auto* file_j = l0_files[j];

      // Check if key ranges overlap
      int cmp_i_largest_j_smallest = internal_comparator_->Compare(
          file_i->largest, file_j->smallest);
      int cmp_j_largest_i_smallest = internal_comparator_->Compare(
          file_j->largest, file_i->smallest);

      // If ranges overlap, merge their groups
      if (cmp_i_largest_j_smallest >= 0 && cmp_j_largest_i_smallest >= 0) {
        unite(i, j);
      }
    }
  }

  // Count the size of each group and track members
  ...
  int max_group_root = -1;
  size_t max_group_size = 0;
  for (const auto& entry : groups) {
    if (entry.second.size() > max_group_size) {
      max_group_size = entry.second.size();
      max_group_root = entry.first;
    }
  }

  // Collect files from the largest group
  std::vector<FileMetaData*> largest_group_files;
  if (max_group_root != -1) {
    for (int file_idx : groups[max_group_root]) {
      largest_group_files.push_back(l0_files[file_idx]);
    }
  }

  return largest_group_files;
}

//jw: Find all groups of overlapping L0 files for intra-L0 compaction
//    Uses Union-Find algorithm to group files with overlapping key ranges
//    Returns vector of groups sorted by total size (largest first)
std::vector<VersionStorageInfo::L0OverlappingGroup>
VersionStorageInfo::FindL0OverlappingGroups(const Comparator* user_cmp) const {
  (void)user_cmp;  // Suppress unused parameter warning
  const auto& l0_files = files_[0];
  int num_files = static_cast<int>(l0_files.size());

  std::vector<L0OverlappingGroup> groups;
  ...
  // Step 1: Check overlap between all pairs of files and unite overlapping ones
  for (int i = 0; i < num_files; i++) {
    // Skip files being compacted
    ...
    for (int j = i + 1; j < num_files; j++) {
      ...
      // Check if key ranges overlap using internal key comparator
      int cmp_i_largest_j_smallest = internal_comparator_->Compare(
          file_i->largest, file_j->smallest);
      int cmp_j_largest_i_smallest = internal_comparator_->Compare(
          file_j->largest, file_i->smallest);
  ...
  // Step 3: Create L0OverlappingGroup objects
  for (const auto& entry : group_map) {
    const auto& file_indices = entry.second;

    // Only consider groups with at least one file
    if (file_indices.empty()) {
      continue;
    }

    std::vector<FileMetaData*> group_files;
    InternalKey group_smallest = l0_files[file_indices[0]]->smallest;
    InternalKey group_largest = l0_files[file_indices[0]]->largest;

    for (int idx : file_indices) {
      group_files.push_back(l0_files[idx]);

      // Update group boundaries
      if (internal_comparator_->Compare(l0_files[idx]->smallest, group_smallest) < 0) {
        group_smallest = l0_files[idx]->smallest;
      }
      if (internal_comparator_->Compare(l0_files[idx]->largest, group_largest) > 0) {
        group_largest = l0_files[idx]->largest;
      }
    }

    groups.emplace_back(std::move(group_files),
                        std::move(group_smallest),
                        std::move(group_largest));
  }

  // Step 4: Sort groups by total size (largest first) for better compaction decisions
  std::sort(groups.begin(), groups.end(),
      [](const L0OverlappingGroup& a, const L0OverlappingGroup& b) {
        uint64_t size_a = 0;
        uint64_t size_b = 0;
        for (auto* f : a.files) {
          size_a += f->fd.GetFileSize();
        }
        for (auto* f : b.files) {
          size_b += f->fd.GetFileSize();
        }
...
void VersionStorageInfo::CalculateBaseBytes(const ImmutableOptions& ioptions,
  ...
  //jw: Conditionally use max overlapping group size based on use_new_flush flag (1=split, 2=bucket)
  if (ioptions.use_new_flush != 0) {
    // Use max overlapping group size for new flush logic
    int max_group_size = CalculateMaxOverlappingGroupSize();
    set_l0_delay_trigger_count(max_group_size);
  } else {
    // Use original L0 file count
