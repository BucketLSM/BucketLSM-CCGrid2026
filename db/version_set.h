...
class VersionStorageInfo {
 public:
  ...
  void EstimateCompactionBytesNeeded(
      const MutableCFOptions& mutable_cf_options,
      const ImmutableOptions& immutable_options); //jw: Added to access use_new_flush
  ...
  //jw: Get all files in the largest overlapping group in L0
  std::vector<FileMetaData*> GetLargestOverlappingGroup() const;
  ...
  //jw: Structure to represent a group of overlapping L0 files
  struct L0OverlappingGroup {
    std::vector<FileMetaData*> files;
    InternalKey smallest;
    InternalKey largest;

    L0OverlappingGroup() = default;
    L0OverlappingGroup(std::vector<FileMetaData*> f, InternalKey s, InternalKey l)
        : files(std::move(f)), smallest(std::move(s)), largest(std::move(l)) {}
  };

  //jw: Find groups of overlapping L0 files for intra-L0 compaction
  //    Returns a vector of groups where files within each group overlap
  std::vector<L0OverlappingGroup> FindL0OverlappingGroups(
      const Comparator* user_cmp) const;
 ...
 private:
  ...
  void UpdateFilesByCompactionPri(const ImmutableOptions& immutable_options,
                                  const MutableCFOptions& mutable_cf_options);
  //jw
  int CalculateMaxOverlappingGroupSize() const;

  //jw: Helper for group-based L0 pending bytes estimation (Option B)
  //    Only counts groups with >= 4 files
  uint64_t EstimateL0CompactionBytesNeededByGroups(
      const MutableCFOptions& mutable_cf_options);
