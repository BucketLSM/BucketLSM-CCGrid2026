...
//jw: Added skip_sync parameter for batch sync optimization
IOStatus WritableFileWriter::Close(const IOOptions& opts, bool skip_sync) {
  ...
  if (use_direct_io()) {
    ...
    //jw: Skip Fsync when skip_sync=true (caller will batch sync later)
    if (interim.ok() && !skip_sync) {
      {
        FileOperationInfo::StartTimePoint start_ts;
        if (ShouldNotifyListeners()) {
          start_ts = FileOperationInfo::StartNow();
