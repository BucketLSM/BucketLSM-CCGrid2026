...
struct DBOptions {
  ...
  CompactionStyleSet calculate_sst_write_lifetime_hint_set = {
   ...
   //jw
   // Flush mode selection:
   // 0 = Original flush (single SST file)
   // 1 = Bucket flush (global bucket-based splitting)
   int use_new_flush = 0;

   //jw
   // Enable parallel flush within bucket flush (requires use_new_flush=1)
   // When enabled, memtable segments are flushed in parallel using multiple threads
   // Default: false (sequential flush)
   bool parallel_split_flush = false;

   //jw
   // Number of buckets to split memtable into when L0 is empty (requires use_new_flush=1)
   // When L0 has no existing files, the first memtable flush will be split into
   // this many buckets to bootstrap parallel L0 compaction groups.
   // Default: 3
   int initial_split_count = 3;

   //jw
   // Enable dynamic bucket boundary adjustment after initial split (requires use_new_flush=1)
   // When enabled, bucket boundaries can be split/merged based on workload pattern
   // When disabled, bucket boundaries remain fixed after initial split
   // Default: true
   bool enable_bucket_adjustment = true;

   //jw
   // Enable parallel L0→L1 compactions (requires use_new_flush=1)
   // Default: true
   bool parallel_l0_compaction = true;

   //jw
   // Enable intra-L0 compaction (L0->L0 compaction when L0 is blocked)
   // When disabled, L0 files will only compact to base level
   bool enable_intra_l0_compaction = true;

   //jw
   // Enable weighted L0 group selection for compaction (requires use_new_flush=1)
   // When enabled, L0 group selection considers both file count and L1 overlap ratio.
   // Score = α * (N / N_max) + (1 - α) * (1 - R / R_max)
   // where N = file count, R = overlap ratio with L1
   // When disabled, uses original logic (select group with most files)
   // Default: true
   bool l0_weight_compaction = true;

   //jw
   // Weight for file count in L0 group selection (requires l0_weight_compaction=true)
   // α in the formula: Score = α * normalized_file_count + (1-α) * normalized_overlap_efficiency
   // Higher value = prioritize reducing L0 file count
   // Lower value = prioritize reducing compaction size (L1 overlap)
   // Valid range: 0.0 to 1.0
   // Default: 0.5 (equal weight)
   double l0_file_count_weight = 0.5;

   //jw
   // Enable range-aware L0 scanning optimization for iterators.
   // When true, L0 SST files are lazily opened based on key range overlap.
   // Only files whose key range overlaps with the current scan position
   // will have iterators created, reducing unnecessary I/O for L0 scans.
   // Default: false
   bool new_range_scan = false;
