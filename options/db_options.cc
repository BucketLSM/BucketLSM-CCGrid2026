    ...
    db_immutable_options_type_info = {
        ...
        {"wal_write_temperature",
         {offsetof(struct ImmutableDBOptions, wal_write_temperature),
          ...
          OptionTypeFlags::kNone}},
        {"use_new_flush",
         {offsetof(struct ImmutableDBOptions, use_new_flush),
          OptionType::kInt32T, OptionVerificationType::kNormal,
          OptionTypeFlags::kNone}}, //jw: 0=original, 1=split, 2=bucket
        {"parallel_split_flush",
         {offsetof(struct ImmutableDBOptions, parallel_split_flush),
          OptionType::kBoolean, OptionVerificationType::kNormal,
          OptionTypeFlags::kNone}}, //jw
        {"initial_split_count",
         {offsetof(struct ImmutableDBOptions, initial_split_count),
          OptionType::kInt, OptionVerificationType::kNormal,
          OptionTypeFlags::kNone}}, //jw
        {"enable_bucket_adjustment",
         {offsetof(struct ImmutableDBOptions, enable_bucket_adjustment),
          OptionType::kBoolean, OptionVerificationType::kNormal,
          OptionTypeFlags::kNone}}, //jw
        {"parallel_l0_compaction",
         {offsetof(struct ImmutableDBOptions, parallel_l0_compaction),
          OptionType::kBoolean, OptionVerificationType::kNormal,
          OptionTypeFlags::kNone}}, //jw
        {"l0_weight_compaction",
         {offsetof(struct ImmutableDBOptions, l0_weight_compaction),
          OptionType::kBoolean, OptionVerificationType::kNormal,
          OptionTypeFlags::kNone}}, //jw
        {"l0_file_count_weight",
         {offsetof(struct ImmutableDBOptions, l0_file_count_weight),
          OptionType::kDouble, OptionVerificationType::kNormal,
          OptionTypeFlags::kNone}}, //jw
        {"new_range_scan",
         {offsetof(struct ImmutableDBOptions, new_range_scan),
          OptionType::kBoolean, OptionVerificationType::kNormal,
          OptionTypeFlags::kNone}}, //jw
          {"is_db_bench",
            {offsetof(struct ImmutableDBOptions, is_db_bench),
             OptionType::kBoolean, OptionVerificationType::kNormal,
             OptionTypeFlags::kNone}},
};
...
ImmutableDBOptions::ImmutableDBOptions(const DBOptions& options)
    : create_if_missing(options.create_if_missing),
      ...
      use_new_flush(options.use_new_flush),  //jw
      parallel_split_flush(options.parallel_split_flush),  //jw
      initial_split_count(options.initial_split_count),  //jw
      enable_bucket_adjustment(options.enable_bucket_adjustment),  //jw
      parallel_l0_compaction(options.parallel_l0_compaction),  //jw
      enable_intra_l0_compaction(options.enable_intra_l0_compaction),  //jw
      l0_weight_compaction(options.l0_weight_compaction),  //jw
      l0_file_count_weight(options.l0_file_count_weight),  //jw
      new_range_scan(options.new_range_scan),  //jw
      core_number(options.core_number), // FEAT
      max_memtable_size(options.max_memtable_size),
      is_db_bench(options.is_db_bench)  { // FEAT
  fs = env->GetFileSystem();
  clock = env->GetSystemClock().get();
  logger = info_log.get();
  stats = statistics.get();
  job_stats = std::make_shared<std::vector<QuicksandMetrics>>(); // FEAT
  flush_stats = std::make_shared<std::vector<FlushMetrics>>(); // FEAT
}
