...
struct ImmutableDBOptions {
  ...
  int use_new_flush; //jw: 0=original, 1=bucket
  bool parallel_split_flush; //jw
  int initial_split_count; //jw
  bool enable_bucket_adjustment; //jw
  bool parallel_l0_compaction; //jw
  bool enable_intra_l0_compaction; //jw
  bool l0_weight_compaction; //jw
  double l0_file_count_weight; //jw
  bool new_range_scan; //jw
  uint64_t core_number; // FEAT
  uint64_t max_memtable_size; // FEAT
  bool is_db_bench;
  std::shared_ptr<std::vector<QuicksandMetrics>> job_stats; // FEAT
  std::shared_ptr<std::vector<FlushMetrics>> flush_stats; // FEAT
