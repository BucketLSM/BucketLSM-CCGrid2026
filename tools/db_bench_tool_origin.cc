...
//jw: Flush mode: 0=original, 1=split (union-find), 2=bucket
DEFINE_int32(use_new_flush, 0, "Flush mode: 0=original, 1=split (union-find), 2=bucket");

DEFINE_bool(parallel_split_flush, false,
            "Enable parallel flush within split flush (requires use_new_flush=1 or 2)"); //jw

DEFINE_bool(parallel_l0_compaction, true,
            "Enable parallel L0->L1 compactions (requires use_new_flush=1 or 2)"); //jw

DEFINE_bool(enable_intra_l0_compaction, true, "Enable intra-L0 compaction (L0->L0)"); //jw

DEFINE_int32(initial_split_count, 3,
             "Number of segments to split memtable when L0 is empty (default: 3)"); //jw

DEFINE_bool(enable_bucket_adjustment, true,
            "Enable dynamic bucket boundary adjustment (requires use_new_flush=2)"); //jw

DEFINE_bool(l0_weight_compaction, true,
            "Enable weighted L0 group selection for compaction (requires use_new_flush=1)"); //jw

DEFINE_double(l0_file_count_weight, 0.5,
              "Weight for file count in L0 group selection (0.0-1.0, requires l0_weight_compaction=true)"); //jw

DEFINE_bool(new_range_scan, false,
            "Enable range-aware L0 scanning optimization for iterators"); //jw
...
// Burst write FLAGS (for BurstWritten function)
DEFINE_int64(burst_size, 1000, "Number of keys per burst in BurstWritten");
DEFINE_int64(burst_interval, 1000, "Microseconds to sleep between bursts");
...
DEFINE_bool(rotating_hot_ranges, false,
            "Enable rotating hot ranges mode: hot ranges shift over time");
DEFINE_int64(rotating_range_interval, 10000000,
             "Number of writes after which hot range rotates to next range");
...
// ReporterAgent class removed - now using the one from report_agent.h
...
struct SharedState {
  ...
  //jw: Track global bytes for throughput monitoring
  std::atomic<uint64_t>* global_bytes_written = nullptr;
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 private:
  ...
  Stats RunBenchmark(int n, Slice name,
    ...
    shared.num_done = 0;
    shared.start = false;
    //jw: Disabled write rate limiter for maximum write throughput
    ...
    //jw: Initialize throughput monitoring
    static std::atomic<bool> monitor_stop(false);
    static std::unique_ptr<std::thread> monitor_thread;
    static std::ofstream throughput_log;
    static std::atomic<uint64_t> global_bytes_written(0);
    static bool monitor_initialized = false;
    static std::mutex monitor_mutex;
    
    //jw: Thread activity monitoring
    static std::unique_ptr<std::thread> thread_monitor_thread;
    static std::ofstream thread_activity_log;
    static std::ofstream level_sizes_log;
    static bool thread_monitor_initialized = false;
    
    uint64_t start_time_micros = FLAGS_env->NowMicros();
    
    // Set global bytes pointer in shared state
    shared.global_bytes_written = &global_bytes_written;
    
    // Start monitoring thread for write operations (only once!)
    if ((name == "fillrandom" || name == "fillseq" || name == "filluniquerandom") && !monitor_initialized) {
      std::lock_guard<std::mutex> lock(monitor_mutex);
      if (!monitor_initialized) {
        monitor_initialized = true;
        monitor_stop = false;
        throughput_log.open("write_throughput.csv");
        if (throughput_log.is_open()) {
          throughput_log << "second,mb_per_sec,bytes_this_second,cumulative_gb\n";
          throughput_log.flush();
        
        monitor_thread.reset(new std::thread([start_time_micros]() {
          uint64_t last_bytes = 0;
          double time_counter = 0.0;  // Use double for 0.1 second intervals
          
          while (!monitor_stop.load()) {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));  // 0.1 second
            if (monitor_stop.load()) break;
            
            uint64_t current_bytes = global_bytes_written.load();
            uint64_t bytes_this_interval = current_bytes - last_bytes;
            
            // Always write a line, even if 0 bytes this interval
            // Scale up to per-second rate (multiply by 10 since interval is 0.1s)
            double mb_per_sec = (bytes_this_interval * 10.0) / (1024.0 * 1024.0);
            double cumulative_gb = current_bytes / (1024.0 * 1024.0 * 1024.0);
            throughput_log << std::fixed << std::setprecision(1) << time_counter 
                         << "," << mb_per_sec << "," 
                         << bytes_this_interval << "," << cumulative_gb << "\n";
            throughput_log.flush();
            
            last_bytes = current_bytes;
            time_counter += 0.1;  // Increment by 0.1 seconds
          }
          
          // Write final data point when stopping
          if (last_bytes != global_bytes_written.load()) {
            uint64_t final_bytes = global_bytes_written.load();
            uint64_t bytes_this_interval = final_bytes - last_bytes;
            // Scale up to per-second rate (multiply by 10 since interval is 0.1s)
            double mb_per_sec = (bytes_this_interval * 10.0) / (1024.0 * 1024.0);
            double cumulative_gb = final_bytes / (1024.0 * 1024.0 * 1024.0);
            throughput_log << std::fixed << std::setprecision(1) << time_counter 
                         << "," << mb_per_sec << "," 
                         << bytes_this_interval << "," << cumulative_gb << "\n";
            throughput_log.flush();
    ...
    //jw: Start thread activity monitoring alongside throughput monitoring
    if (!thread_monitor_initialized && db_.db != nullptr) {
      std::lock_guard<std::mutex> lock(monitor_mutex);
      if (!thread_monitor_initialized) {
        thread_monitor_initialized = true;

        thread_activity_log.open("thread_activity.csv");
        level_sizes_log.open("level_sizes.csv");

        if (thread_activity_log.is_open() && level_sizes_log.is_open()) {
          thread_activity_log << "timestamp,thread_id,thread_type,operation,stage,compaction_bytes,flush_bytes,input_level,output_level\n";
          thread_activity_log.flush();

          level_sizes_log << "timestamp,level,num_files,size_bytes\n";
          level_sizes_log.flush();

          DB* db_ptr = db_.db;  // Capture db pointer instead of this
          thread_monitor_thread.reset(new std::thread([db_ptr, start_time_micros]() {
            while (!monitor_stop.load()) {
              std::this_thread::sleep_for(std::chrono::milliseconds(100)); // 100ms interval
              if (monitor_stop.load()) break;

              double elapsed = (FLAGS_env->NowMicros() - start_time_micros) / 1000000.0;

              // Collect thread activity
              std::vector<ThreadStatus> thread_list;
              Status s = db_ptr->GetEnv()->GetThreadList(&thread_list);
              if (s.ok()) {
                for (const auto& ts : thread_list) {
                  ...
                  if (ts.operation_type == ThreadStatus::OP_COMPACTION) {
                    compaction_bytes = ts.op_properties[ThreadStatus::COMPACTION_BYTES_WRITTEN];
                    // Extract compaction levels from COMPACTION_INPUT_OUTPUT_LEVEL property
                    uint64_t levels = ts.op_properties[ThreadStatus::COMPACTION_INPUT_OUTPUT_LEVEL];
                    input_level = (levels >> 32) & 0xFFFFFFFF;
                    output_level = levels & 0xFFFFFFFF;
                  } else if (ts.operation_type == ThreadStatus::OP_FLUSH) {
                    flush_bytes = ts.op_properties[ThreadStatus::FLUSH_BYTES_WRITTEN];
                    // Flush always writes to L0
                    output_level = 0;
                  }

                  thread_activity_log << std::fixed << std::setprecision(2) << elapsed
                                    << "," << ts.thread_id
                                    << "," << static_cast<int>(ts.thread_type)
                                    << "," << static_cast<int>(ts.operation_type)
                                    << "," << static_cast<int>(ts.operation_stage)
                                    << "," << compaction_bytes
                                    << "," << flush_bytes
                                    << "," << input_level
                                    << "," << output_level << "\n";
              ...
              // Collect level sizes (for all 7 levels: L0-L6)
              for (int level = 0; level < 7; level++) {
                std::string num_files_str;
                std::string level_size_str;

                // Get number of files at this level
                std::string num_files_property = "rocksdb.num-files-at-level" + std::to_string(level);
                if (db_ptr->GetProperty(num_files_property, &num_files_str)) {
                  uint64_t num_files = std::stoull(num_files_str);

                  // Get total size at this level (in bytes)
                  // We'll use GetProperty for "rocksdb.total-sst-files-size" or estimate from num-files
                  // For simplicity, use the aggregated stats
                  std::string stats;
                  uint64_t level_size = 0;

                  if (db_ptr->GetProperty("rocksdb.stats", &stats)) {
                    // Parse stats string to find level size
                    std::string level_marker = "L" + std::to_string(level);
                    size_t pos = stats.find("\n  " + level_marker + " ");
                    if (pos != std::string::npos) {
                      // Find the size column (3rd column after level)
                      size_t line_end = stats.find('\n', pos + 1);
                      std::string line = stats.substr(pos + 1, line_end - pos - 1);
                      std::istringstream iss(line);
                      std::string token;
                      int col = 0;
                      while (iss >> token && col < 3) {
                        if (col == 2) {
                          // This should be the size (e.g., "1.23")
                          try {
                            double size_val = std::stod(token);
                            // Next token should be unit (KB, MB, GB)
                            std::string unit;
                            if (iss >> unit) {
                              if (unit == "KB") level_size = static_cast<uint64_t>(size_val * 1024);
                              else if (unit == "MB") level_size = static_cast<uint64_t>(size_val * 1024 * 1024);
                              else if (unit == "GB") level_size = static_cast<uint64_t>(size_val * 1024 * 1024 * 1024);
                            }
                          } catch (...) {}
                        }
                        col++;
                  ...
                  level_sizes_log << std::fixed << std::setprecision(2) << elapsed
                                 << "," << level
                                 << "," << num_files
                                 << "," << level_size << "\n";
                }
    ...
    if (FLAGS_report_interval_seconds > 0) {
      if (FLAGS_DOTA_enabled || FLAGS_FEA_enable || FLAGS_TEA_enable) {
        // Use ReporterAgentWithTuning for FEAT/DOTA
    ...
    //jw: Stop monitoring threads after benchmark completes
    monitor_stop.store(true);

    // Join the monitoring threads if they exist
    if (monitor_thread && monitor_thread->joinable()) {
      monitor_thread->join();
      monitor_thread.reset();
    }
    if (thread_monitor_thread && thread_monitor_thread->joinable()) {
      thread_monitor_thread->join();
      thread_monitor_thread.reset();
    }
    ...
    if (thread_activity_log.is_open()) {
      thread_activity_log.close();
    }
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    options.use_new_flush = FLAGS_use_new_flush;
    options.parallel_split_flush = FLAGS_parallel_split_flush; //jw
    options.parallel_l0_compaction = FLAGS_parallel_l0_compaction; //jw
    options.enable_intra_l0_compaction = FLAGS_enable_intra_l0_compaction; //jw
    options.initial_split_count = FLAGS_initial_split_count; //jw
    options.enable_bucket_adjustment = FLAGS_enable_bucket_adjustment; //jw
    options.l0_weight_compaction = FLAGS_l0_weight_compaction; //jw
    options.l0_file_count_weight = FLAGS_l0_file_count_weight; //jw
    options.new_range_scan = FLAGS_new_range_scan; //jw
    ...
    //jw: Disabled delayed_write_rate to prevent L0 slowdown throttling
    // options.delayed_write_rate = FLAGS_delayed_write_rate;
    options.delayed_write_rate = 0;  //jw: Force to 0 (no limit)
  ...
  void InitializeOptionsGeneral(Options* opts, ToolHooks& hooks) {
    ...
    //jw: Disabled sine_write_rate for consistent maximum throughput
    // if (FLAGS_sine_write_rate) {
    //   FLAGS_benchmark_write_rate_limit = static_cast<uint64_t>(SineRate(0));
    // }

    //jw: Disabled RocksDB internal rate limiter for maximum write throughput
  ...
  void OpenDb(Options options, ToolHooks& hooks, const std::string& db_name,
    ...
    //jw: Enable thread tracking for monitoring
    options.enable_thread_tracking = true;
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    ...
    while ((num_per_key_gen != 0) && !duration.Done(entries_per_batch_)) {
      ...
      //jw: Disabled write rate limiting in DoWrite for maximum throughput
      ...
      //jw: Update global bytes in real-time for each batch
      if (thread->shared->global_bytes_written) {
        thread->shared->global_bytes_written->fetch_add(batch_bytes);
      }
      if (FLAGS_sine_write_rate) {
        uint64_t now = FLAGS_env->NowMicros();
    ...
    //jw: Global bytes already updated in real-time during the loop
  }
...
// optimization, so skip the check of the max level.
  ...
  class GenerateTwoTermExpKeys {
   public:
    ...
    int64_t DistGetKeyID(int64_t ini_rand, double key_dist_a,
                         double key_dist_b, bool is_write = false) {
      ...
      // ROTATING HOT RANGES: Shift keyrange_id based on current phase
      if (FLAGS_rotating_hot_ranges && is_write) {
        // Update phase based on write count
        if (FLAGS_rotating_range_interval > 0) {
          current_phase_ = write_count_ / FLAGS_rotating_range_interval;
        }

        // Rotate keyrange_id by phase offset
        // This makes different key ranges hot in different phases
        int64_t phase_offset = current_phase_ % keyrange_num_;
        keyrange_id = (keyrange_id + phase_offset) % keyrange_num_;
  ...
  void MixGraph(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      ...
      // Determine query type FIRST (for rotating hot ranges)
      int query_type = query.GetType(rand_v);
      bool is_write = (query_type == 1);  // query_type 1 = Put
      ...
      if (use_random_modeling) {
        ...
        key_rand =
            gen_exp.DistGetKeyID(ini_rand, FLAGS_key_dist_a, FLAGS_key_dist_b, is_write);
      } else {
      ...
      if (FLAGS_sine_mix_rate &&
        ...
        //jw: Disabled dynamic rate limiting in mixgraph for maximum throughput
        // if (read_rate > 0) {
      ...
      if (query_type == 0) {
        ...
        //jw: Disabled write rate limiting in mixgraph PUT for maximum throughput
