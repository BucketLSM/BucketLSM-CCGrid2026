...
SystemScores DOTA_Tuner::ScoreTheSystem() {
  ...
  auto flush_thread_idle_ptr = env_->GetThreadPoolWaitingTime(Env::HIGH);
  auto compaction_thread_idle_ptr = env_->GetThreadPoolWaitingTime(Env::LOW);

  std::unordered_map<int, uint64_t> thread_idle_time;

  if (flush_thread_idle_ptr != nullptr) {
    auto& flush_thread_idle_list = *flush_thread_idle_ptr;
  ...
  if (compaction_thread_idle_ptr != nullptr) {
    auto& compaction_thread_idle_list = *compaction_thread_idle_ptr;
    uint64_t temp = compaction_thread_idle_list.size();
